...
for L1_MSHRS in {1..15}; do
    for L2_MSHRS in {1..256}; do
...
for L1_MSHRS in {1..15}; do
    for L2_MSHRS in {1..256}; do
        python spmv_multistage.py benchmark -r $ROWS -c $COLS --l1-mshrs "$L1_MSHRS" --l2-mshrs "$L2_MSHRS" --disable-l1-ipp --disable-l1-npp -d 0.0005 --repetitions 8
    done
done
