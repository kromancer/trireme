...
int main() {
    ...
    // echo x | sudo tee /sys/kernel/mm/hugepages/hugepages-2048kB/nr_hugepages
    // Where x is the number of streams
    ...
    for (int s = 0; s < NUM_OF_STREAMS; s++) {
        for (volatile char *a = addr[s]; a < addr[s] + PAGE_SIZE; a += CL) {
            *a = 0;
        }
    }
    ...
    // Flush the caches
    for (int s = 0; s < NUM_OF_STREAMS; s++) {
        for (const char *a = addr[s]; a < addr[s] + PAGE_SIZE; a += CL) {
            _mm_clflush(a);
        }
    }
